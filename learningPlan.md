...
- [ ] 메인 페이지 전체 UI (HTML,CSS)
  ...
  - [ ] 헤더 영역
    ...
    - [ ] 상단의 헤더 영역은 가계부의 모든 화면에서 상단 중앙에 표시한다.
    - [ ] 헤더 왼쪽
      - [ ] 로고를 표시한다.
      - [ ] 클릭 시 메인 화면으로 이동
    - [ ] 헤더 중앙
      - [ ] 현재 연월을 표시한다.
      - [ ] 좌우 화살표를 클릭하면 이전 달과, 다음달로 이동한다.
      - [ ] 하단 내역 리스트도 같이 갱신한다.
    - [ ] 헤더 오른쪽
      - [ ] 내역/달력/통계 탭을 표시한다.
...
- [ ] 새로운 내역 박스
  - [ ] 일자
    - [ ] 날짜 인풋 박스의 기본값을 오늘 날짜로 한다. 수정할 수 있도록 한다.
  - [ ] 금액
    - [ ] 마이너스, 플러스 버튼 - 마이너스는 지출, 플러스는 수입을 의미한다.
      - [ ] 버튼을 클릭하면 마이너스 -> 플러스, 플러스 -> 마이너스 버튼으로 바뀐다.
      - [ ] 버튼 상태에 따라 지출, 수입을 입력할 수 있다.
      - [ ] 버튼 상태에 따라 분류 셀렉트 박스가 다르게 표시된다. 마이너스일 때 지출 카테고리, 플러스일 때 수입 카테고리
      - [ ] 지출 버튼(마이너스)이 기본값으로 한다.
    - [ ] 금액을 입력할 때, 자동으로 세자리마다 쉼표가 찍힌다.
  - [ ] 내용
    - [ ] 해당 내역에 대한 내용을 입력할 수 있다.
  - [ ] 결제수단
